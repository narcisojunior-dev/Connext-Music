...
import { ARTIST_UNKNOWN, extractMetadata } from '@/services/file/metadata-extractor';
...
/**
 * Preenche titulo e artista a partir do nome do arquivo quando a tag nao serve.
 *
 * Roda depois da varredura, e nao dentro de `processAudioFile`, porque a
 * decisao precisa da pasta inteira: `01 - A - B.mp3` sozinho nao diz se A e o
 * titulo ou o artista, e o que responde isso e qual dos lados se repete entre
 * as faixas vizinhas.
 *
 * So mexe em faixa cuja tag e fraca — sem artista e com titulo que parece
 * gerado por maquina (`music 8`, `track 3`). Uma tag completa continua
 * vencendo o nome do arquivo, que e o comportamento certo: quem preencheu as
 * tags quis aquilo.
 */
function applyFileNameFallback(tracks: Track[]): Track[] {
  const porPasta = new Map<string, Track[]>();
  for (const track of tracks) {
    const chave = track.folderPath ?? '';
    const lista = porPasta.get(chave);
    if (lista) lista.push(track);
    else porPasta.set(chave, [track]);
  }

  const corrigidas = new Map<string, Track>();

  for (const faixas of porPasta.values()) {
    const resolvidos = resolveNames(faixas.map((f) => f.fileName));

    faixas.forEach((faixa, i) => {
      const semArtista = faixa.artist === ARTIST_UNKNOWN;
      if (!looksAutoGenerated(faixa.title, semArtista ? '' : faixa.artist)) return;

      const { title, artist } = resolvidos[i];
      if (!title) return;

      corrigidas.set(faixa.id, {
        ...faixa,
        title,
        artist: artist ?? faixa.artist,
        // O numero da tag tambem nao presta quando o resto dela nao presta —
        // nos arquivos que motivaram isto, todos vinham como 1. O nome do
        // arquivo tem a numeracao de verdade.
        trackNumber: parseFileName(faixa.fileName).trackNumber ?? faixa.trackNumber,
      });
    });
  }

  return corrigidas.size === 0 ? tracks : tracks.map((t) => corrigidas.get(t.id) ?? t);
}
...
export async function scanMusicLibrary({
  ...
  const unique = applyFileNameFallback(removeDuplicates(tracks));
